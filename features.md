...
- **Built-in middlewares (`middlewares`)**
  ...
  - `file_upload::file_upload(upload_dir)`
    - Handles binary uploads and browser `multipart/form-data` (extracts and saves ALL file parts and text fields).
    - Ensures upload directory exists; generates UUID-based filename; detects extension via `infer`.
    - Saves files asynchronously with `tokio::fs`.
    - Injects comprehensive request data: `uploaded_file_count`, `uploaded_files`, and backwards compatibility keys.
    - Maps form field names to generated UUID filenames for easy access.
