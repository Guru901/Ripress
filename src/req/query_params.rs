...
/// ```rust
/// use std::collections::HashMap;
/// use ripress::req::query_params::{QueryParams, QueryParamError};
///
...
///         println!("Parameter '{}' not found", param);
///     }
///     _ => {}
/// }
///
/// // Parse error
/// match params.get_int("page") {
///     Err(QueryParamError::ParseError { param, value, target_type }) => {
///         println!("Failed to parse '{}' as {}: '{}'", param, target_type, value);
///     }
///     _ => {}
/// }
/// ```
...
pub enum QueryParamError {
    ...
    ///
    /// match params.get_int("offset") {
    ///     Err(QueryParamError::NotFound(param_name)) => {
    ///         assert_eq!(param_name, "offset");
    ///         println!("Parameter '{}' was not provided", param_name);
    ///     }
    ///     _ => unreachable!(),
    /// }
    /// ```
    ...
    /// match params.get_bool("active") {
    ///     Err(QueryParamError::ParseError { param, value, target_type }) => {
    ///         assert_eq!(param, "active");
    ...
    ///
    /// ```rust
    /// // This would be generated by hypothetical strict single-value methods
    ///
    /// use ripress::req::query_params::{QueryParams, QueryParamError};
    ///
    /// let error = QueryParamError::MultipleValues {
    ///     param: "tags".to_string(),
    ///     values: vec!["rust".to_string(), "web".to_string(), "backend".to_string()],
    /// };
    ///
    /// match error {
    ///     QueryParamError::MultipleValues { param, values } => {
    ///         println!("Parameter '{}' has {} values: {:?}", param, values.len(), values);
    ///     }
    ///     _ => {}
    /// }
    /// ```
