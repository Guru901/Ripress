...
/// Error type for query parameter parsing failures
...
impl QueryParams {
    ...
    pub fn get_parsed<T>(&self, name: &str) -> Result<T, QueryParamError>
    where
        T: FromStr,
        T::Err: std::fmt::Debug,
    {
        ...
        value.parse::<T>().map_err(|_| QueryParamError::ParseError {
    ...
    pub fn get_all_parsed<T>(&self, name: &str) -> Result<Vec<T>, QueryParamError>
    where
        T: FromStr,
        T::Err: std::fmt::Debug,
    {
    ...
    pub fn get_int(&self, name: &str) -> Result<i32, QueryParamError> {
        self.get_parsed::<i32>(name)
    }

    /// Get parameter as 64-bit integer
    pub fn get_i64(&self, name: &str) -> Result<i64, QueryParamError> {
        self.get_parsed::<i64>(name)
    }

    /// Get parameter as unsigned integer
    pub fn get_uint(&self, name: &str) -> Result<u32, QueryParamError> {
        self.get_parsed::<u32>(name)
    }

    /// Get parameter as boolean
    /// Supports: "true"/"false", "1"/"0", "yes"/"no", "on"/"off"
    pub fn get_bool(&self, name: &str) -> Result<bool, QueryParamError> {
        let value = self
            .get(name)
            .ok_or_else(|| QueryParamError::NotFound(name.to_string()))?;
        ...
        match value.to_lowercase().as_str() {
            ...
            _ => Err(QueryParamError::ParseError {
                param: name.to_string(),
                value: value.to_string(),
    ...
    pub fn get_float(&self, name: &str) -> Result<f64, QueryParamError> {
